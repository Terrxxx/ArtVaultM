...
def comment_to_dict(c: Comment, mentions: Optional[List[User]] = None) -> dict:
    return {
        "id": c.id,
        "asset_id": c.asset_id,
        ...
        # 正文里 @ 到的人：前端据此把 @昵称 变成可点的主页链接
        "mentions": [user_brief(u) for u in (mentions or [])],
...
def list_comments(
    ...
    # 用户表只查一次，逐条评论在里面找 @ 到的人
    users = db.query(User).filter(User.status == "active").all()
    return [
        comment_to_dict(c, notify.extract_mentions(db, c.content, users=users))
        for c in comments
    ]
...
def add_comment(
    ...
    db.commit()
    db.refresh(comment)
    data = comment_to_dict(comment, notify.extract_mentions(db, comment.content))
...
def delete_comment(
    ...
    # 只能删自己的；高级管理员例外，可以删任何人的
    if comment.user_id != user.id and not is_super_admin(user):
